<script>
  ...
  const getSchoolName = () => {
    let parts = currentRoute.namedParams.school.split('_')
  ...
  onMount(async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.API_ROOT}/api/lunch-week-public/${currentRoute.namedParams.school}/${currentRoute.namedParams.weekOf}`
      )
...
<div>
  <h1 class="title is-4">
    {getSchoolName()} Lunch Menu for the Week of {currentRoute.namedParams
      .weekOf}
  </h1>
  {#if loading}
    <div class="section">
